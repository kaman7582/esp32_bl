...
void param_init(wifi_config_t * wifi_cfg)
{
	char ssid[64]={"qiao qiao house"};
	char key[64]={"198422hk"};
	memset(wifi_cfg,0,sizeof(wifi_config_t));
	memcpy(wifi_cfg->sta.ssid,ssid,strlen(ssid));
	memcpy(wifi_cfg->sta.password,key,strlen(key));
...
void wifi_stat_init()
{
	...
	//step 1 TCP/IP INIT
	tcpip_adapter_init();
	//step 2 init even loop
	ESP_ERROR_CHECK( esp_event_loop_init(event_handler, NULL) );
	//step 3 wifi config init
...
static void update_device_info(esp_bt_gap_cb_param_t *param)
{
    ...
    ESP_LOGI(GAP_TAG, "Device found: %s number:%d", bda2str(param->disc_res.bda, bda_str, 18),param->disc_res.num_prop);
    for (int i = 0; i < param->disc_res.num_prop; i++) {
	    ...
	    ESP_LOGI(GAP_TAG,"blue device type\n" );
        default:
            break;
        }
    }
